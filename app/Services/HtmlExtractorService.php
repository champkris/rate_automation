...
{
    /**
     * Extract rate cards from HTML email body
     */
    public function extract(Email $email): array
    {
        try {
            ...
            foreach ($tables as $table) {
                $rateType = $this->determineRateType($email->subject ?? '', $html);
                $extractedRates = $this->extractFromTable($table, $xpath, $email->id, $rateType);
                $rateCards = array_merge($rateCards, $extractedRates);
            }

            ProcessingLog::logSuccess(
                'html_extract',
                "Extracted " . count($rateCards) . " rate cards from HTML email",
                $email->id,
            ...
            ProcessingLog::logFailure(
                'html_extract',
                "Failed to extract data from HTML email",
                $e,
                $email->id
            );
    ...
    /**
     * Extract rate data from a single table
     */
    private function extractFromTable($table, DOMXPath $xpath, int $emailId, ?string $rateType): array
    {
    ...
    /**
     * Determine rate type from email subject or HTML content
     */
    private function determineRateType(string $emailSubject, string $html): ?string
    {
        $text = strtoupper($emailSubject . ' ' . strip_tags($html));
